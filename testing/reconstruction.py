...
class ReconstructionTest():
    def __init__(self, denormalize):
        self.denomalize = denormalize

    def show_reconstructed_images(self, model, dataloader, save_path):
        """
        ...
        for X, y in dataloader:
            for i in range(5):
                image_orig = torch.permute(self.denormalize(X.cuda()), (0,2,3,1))
                model_reconstructed = model(X.cuda())
                if type(model_reconstructed) is tuple:
                    model_reconstructed = model_reconstructed[0]
                image_regen = torch.permute(torch.clamp(self.denormalize(model_reconstructed), 0, 1), (0,2,3,1))
    ...
    def average_l2_dist(self, model, dataloader):
        total_l2 = 0
        count = 0
        model.eval()
        for X, y in tqdm(dataloader):
            image_orig = self.denormalize(X.cuda())
            model_reconstructed = model(X.cuda())
            if type(model_reconstructed) is tuple:
                model_reconstructed = model_reconstructed[0]
            image_regen = torch.clamp(self.denormalize(model_reconstructed), 0, 1)
